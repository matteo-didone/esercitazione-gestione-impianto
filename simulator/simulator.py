...
class Machine:
    ...
    def simulate_variable_data(self, dt: float) -> dict[str, float]:
        ...
        dt: elapsed time (s) since last sample.
        """
        ...
        dT = (
            power * self.heat_coeff
            - (self.current_temp - self.ambient_temp) / self.thermal_time_const
        ) * dt
...
def publish_tracking_event(
    ...
    mqtt_client.publish(
        f"/plant/tracking/{entity}",
        {"entity": entity, "event": event, "data": data, "timestamp": timestamp},
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        for job_idx, job in enumerate(plan, 1):
            ...
            publish_tracking_event(
                mqtt_client, "Warehouse_01", "deposit",
